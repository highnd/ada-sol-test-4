...
import { OTHER_PRODUCTS } from "@/data/CarouselsData";

const CAROUSEL_WRAPPER_CLASSES = "relative ";

const SLIDE_CLASSES =
  "flex-[0_0_50%] sm:flex-[0_0_40%] xl:flex-[0_0_20%] px-3 sm:px-4 py-12 flex flex-col items-center justify-center";
...
export const OtherProductCarousel: React.FC = () => {
        ...
        <div className={CAROUSEL_WRAPPER_CLASSES}>
            ...
            <EmblaCarousel onSelect={onSelect}>
              {OTHER_PRODUCTS.map((item) => (
                <div key={item.id} className={SLIDE_CLASSES}>
                  <div className="w-44 h-44 rounded-full bg-[#FF4C00] mb-4"></div>

                  <p className="mx-auto text-black bold-fanum-font text-center">
                    {item.title}
                  </p>
                </div>
              ))}
          ...
          <div className={CONTROLS_CONTAINER_CLASSES}>
            ...
            <div className=" w-full flex items-center justify-center  ">
              <button className="py-2 px-8 bg-[#FF4C00]  text-white hover:opacity-50  cursor-pointer rounded-xl">
                محصولات بیشتر
              </button>
            </div>
          </div>
